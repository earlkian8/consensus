export const buildAIResults = (auditRows) => {
  ...
  auditRows.forEach((entry) => {
    ...
    if (pct >= 50) {
      newQty = Math.max(1, Math.round(entry.planned * 0.4));
      dir = "down";
      adjustedCount += 1;
      totalSaved += Math.round((entry.planned - newQty) * (entry.cost || 0));
      recommendations.push(
        `${entry.name} had ${pct}% excess - reduce by about 60% next run (${entry.planned} -> ${newQty} ${entry.excUnit}).`,
      );
    } else if (pct > 30) {
      newQty = Math.max(1, Math.round(entry.planned * 0.6));
      dir = "down";
      adjustedCount += 1;
      totalSaved += Math.round((entry.planned - newQty) * (entry.cost || 0));
      recommendations.push(
        `${entry.name} had ${pct}% excess - reduce by about 40% (${entry.planned} -> ${newQty} ${entry.excUnit}).`,
      );
    } else if (pct >= 15) {
      newQty = Math.max(1, Math.round(entry.planned * 0.8));
      dir = "down";
      adjustedCount += 1;
      totalSaved += Math.round((entry.planned - newQty) * (entry.cost || 0));
      recommendations.push(
        `${entry.name} had ${pct}% excess - reduce by about 20% (${entry.planned} -> ${newQty} ${entry.excUnit}).`,
      );
    } else if (pct < 5) {
      newQty = Math.round(entry.planned * 1.1);
      dir = "up";
      recommendations.push(
        `${entry.name} sold out - add about 10% buffer (${entry.planned} -> ${newQty} ${entry.excUnit}).`,
      );
    } else {
      recommendations.push(
    ...
    if (
      entry.unitLiquid &&
      entry.liquidPlanned > 0 &&
      entry.excessLiquid !== undefined
    ) {
      ...
      if (liquidPct >= 50) {
        newLiquidQty = Math.max(0.5, Math.round(liquidPlanned * 0.4 * 10) / 10);
        liquidDir = "down";
        recommendations.push(
          `${entry.name} soup had ${liquidPct}% excess - reduce liquid by about 60% (${liquidPlanned} -> ${newLiquidQty} ${entry.unitLiquid}).`,
        );
      } else if (liquidPct > 30) {
        newLiquidQty = Math.max(0.5, Math.round(liquidPlanned * 0.6 * 10) / 10);
        liquidDir = "down";
        recommendations.push(
          `${entry.name} soup had ${liquidPct}% excess - reduce liquid by about 40% (${liquidPlanned} -> ${newLiquidQty} ${entry.unitLiquid}).`,
        );
      } else if (liquidPct >= 15) {
        newLiquidQty = Math.max(0.5, Math.round(liquidPlanned * 0.8 * 10) / 10);
        liquidDir = "down";
        recommendations.push(
          `${entry.name} soup had ${liquidPct}% excess - reduce liquid by about 20% (${liquidPlanned} -> ${newLiquidQty} ${entry.unitLiquid}).`,
        );
      } else if (liquidPct < 5) {
        newLiquidQty = Math.round(liquidPlanned * 1.1 * 10) / 10;
        liquidDir = "up";
        recommendations.push(
          `${entry.name} soup sold out - add about 10% buffer (${liquidPlanned} -> ${newLiquidQty} ${entry.unitLiquid}).`,
        );
      } else {
        recommendations.push(
